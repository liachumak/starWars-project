...
function getVehicles(page, create = false){
...
function showAllVehicles(data){
	...
	data.forEach(element => {
		let str = `<div class="card mb-3">
							...
							<img src="https://starwars-visualguide.com/assets/img/vehicles/${element.url.match(/\/([0-9]*)\/$/)[1]}.jpg" class="d-block user-select-none">
						</div>`;
...
function activePagination(){
	...
	for(let i = 0; i < page.length; i++){
		page[i].addEventListener('click', function(){
			...
			getVehicles(this.firstElementChild.textContent);
		})
	}
}
...
function showDetails(data, url){
	...
	const {name, model, manufacturer, vehicle_class, cost_in_credits, max_atmosphering_speed, length, crew, passengers} = data;
	title.textContent = name;
	li[0].textContent = model;
	li[1].textContent = manufacturer;
	li[2].textContent = vehicle_class;
